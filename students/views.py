...
def student_create(request):
    ...
    if request.method == 'POST':
        ...
        student = Student.objects.create(
            ...
            parent_email=p.get('parent_email', ''), parent_name=p.get('parent_name', ''),
...
def parent_portal(request):
    ...
    if email:
        ...
        student = Student.objects.filter(parent_email__iexact=email, active=True).first()
        if student:
            works       = StudentWork.objects.filter(student=student, is_public=True)
            grades      = student.grades.order_by('-date')[:20]
            msgs        = student.messages.order_by('-sent_at')[:10]
